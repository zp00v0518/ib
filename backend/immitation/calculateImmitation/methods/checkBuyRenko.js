...
function checkBuyRenko(minMaxArr, settings) {
  let renkoChart = getRenkoChart(minMaxArr, settings.renkoGrow);

  let templateArr = settings.renkoArr;

  const result = templateArr.some((item) => {
    if (renkoChart.length < item.length) return false;
    const checkArr = renkoChart.splice(0 - item.length);
    const flag = checkArr.every((i, index) => {
      return !!i === !!item[index];
    });
    return flag;
